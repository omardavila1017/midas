...
- **Identidad UI:** `src/contexts/AuthContext.tsx` (`useAuth → { email, role, expiresAt, can(tab) }`). `can(tab) = canAccess(email, sessionRole, tab)`; el **rol efectivo** combina el override del registro (p.ej. admin promovió al usuario) con el rol de la sesión. Se reevalúa en vivo vía `subscribeAccessChanged`.
