...
- **Sesión / Contraseñas:** sin cambios. `Login.tsx` (`AuthGate`) consulta `GET /api/auth/session` y `POST /api/auth/login` (`credentials:'include'`); cookie `HttpOnly`; sin tokens en cliente. `authApi.ts` (`changePassword`/`requestPasswordReset`/`completePasswordReset`/`sendUserPasswordReset`) `normalizeRole` ahora usa `coerceRole`. `localAuth.ts` (`roleForUser`→`coerceRole`, nuevo `listLocalUsers()` para siembra).
