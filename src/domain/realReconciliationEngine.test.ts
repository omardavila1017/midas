...
describe('reconcileRealCollections — match con tolerancia (capa 2)', () => {
  it('cruza con diferencia de 0.3% (comisión bancaria)', () => {
    ...
    expect(result.matches[0].status).toBe('cobrada-banco');
    expect(result.matches[0].matchTier).toBe('tolerance');
  });
...
describe('reconcileRealCollections — KPIs', () => {
  it('% facturas cruzadas y % abonos cruzados calculan correctamente', () => {
    const facturas: CobranzaRecord[] = [
      makeFactura({ cia: '00011', noFactura: 'F-1', noCliente: 'C-1', nombreCliente: 'A', importeBrutoPesos: 100 }),
      makeFactura({ cia: '00011', noFactura: 'F-2', noCliente: 'C-2', nombreCliente: 'B', importeBrutoPesos: 200 }),
    ];
    const abonos = [
      makeAbono({ cia: '00011', cuenta: '1', fechaOperacion: '2026-02-01', importe: 100, concepto: 'A' }),
