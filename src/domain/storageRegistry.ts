...
export const MIDAS_STORAGE_REGISTRY: StorageEntry[] = [
  ...
  { key: 'midas.users.registry.v6', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios (correo → rol admin/user + permisos por módulo). Capa UX de RBAC; se siembra del JSON local / VITE_USER_ROLES. v6 = re-siembra forzada tras alta de david.betancourt (user, solo taxes) + antonio.palomo (admin → user con todos los módulos funcionales).' },
