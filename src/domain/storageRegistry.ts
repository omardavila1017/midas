...
export const MIDAS_STORAGE_REGISTRY: StorageEntry[] = [
  ...
  { key: 'midas.auth.session.v2', scope: 'localStorage', owner: 'services/midasSession.ts', description: 'Marcador de sesión del modo ONLINE (WS/midas/usuarios): correo + rol + permisos + expiración (TTL). Solo identidad — NUNCA token ni contraseña. Se limpia en logout.' },
  ...
  { key: 'midas.auth.localOverrides.v2', scope: 'localStorage', owner: 'services/localAuth.ts', description: 'Overlay de contraseñas (email → hash) en modo auth LOCAL: definida por un usuario en su primer ingreso, cambio propio, o fijada por un admin. v2 = reset total (el JSON ya no siembra contraseñas); es la única fuente de credenciales en modo local.' },
  { key: 'midas.users.registry.v7', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios (correo → rol admin/user + permisos por módulo). Capa UX de RBAC; se siembra del JSON local / VITE_USER_ROLES. v7 = re-siembra forzada tras dar a blanca.reyes acceso a todo el módulo de Ingresos (se agrega netflow).' },
  { key: 'midas.users.registry.v6', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios previo (alta de david.betancourt user/taxes + antonio.palomo admin→user); abandonado al subir a v7. Se purga en reset.', legacy: true },
  { key: 'midas.users.registry.v5', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios previo (alta de maximiliano.rodriguez admin); abandonado al subir a v6. Se purga en reset.', legacy: true },
  { key: 'midas.users.registry.v4', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios previo (alta de jesus.villarreal + promoción de varios a admin); abandonado al subir a v5. Se purga en reset.', legacy: true },
  { key: 'midas.users.registry.v3', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios previo (roster + roles + permisos hardcodeados); abandonado al subir a v4. Se purga en reset.', legacy: true },
  { key: 'midas.users.registry.v2', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios previo (roster + roles hardcodeados, sin permisos); abandonado al subir a v3. Se purga en reset.', legacy: true },
  { key: 'midas.users.registry.v1', scope: 'localStorage', owner: 'modules/users/services/accessControlStore.ts', description: 'Registro de usuarios previo (roster viejo); abandonado al subir a v2. Se purga en reset.', legacy: true },
