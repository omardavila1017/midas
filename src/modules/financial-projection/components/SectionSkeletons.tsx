...
export function ChartSkeleton({ height = 340 }: { height?: number }) {
  return (
    <div
      className="rounded-xl bg-[var(--gray-50)]"
      style={{ height }}
      aria-hidden="true"
    >
      <div className="skeleton h-full w-full rounded-xl opacity-60" />
    </div>
  );
}
    ...
    <div className="px-4 py-3" aria-hidden="true">
      <div className="space-y-2">
        {Array.from({ length: rows }).map((_, idx) => (
          <div key={idx} className="skeleton h-9 w-full rounded-lg opacity-60" />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-2 gap-3 p-4 md:grid-cols-4" aria-hidden="true">
      {Array.from({ length: items }).map((_, idx) => (
        <div key={idx} className="rounded-xl border border-[var(--gray-200)] bg-[var(--gray-50)] px-3 py-3">
