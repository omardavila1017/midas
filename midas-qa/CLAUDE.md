...
- **ROL (CITI) — proyectado como ingreso real con cobro futuro.** `fetchRolRange` trae los viajes ejecutados y se cachean en IDB (`rolRecords`). El cruce `buildRolCobranzaCross` (`src/domain/rolCobranzaMatch.ts`) los empata contra cobranza por `factura`/`uuidFiscal` y alimenta `RolCobranzaPanel` dentro de la pestaña Cobranza (facturado / predicho / huérfano). Los viajes predichos (ejecutados, aún no facturados) entran al motor canónico como líneas `rol:` vía `buildRolProjectedInflows` → `canonicalProjection.ts:918`, fechados por la regla de pago del catálogo de clientes (sincronizada desde el API de cobranza: `creditDays`, `paymentDayName` CC13, `frequency` CC17). Importante: **ROL también entra al escenario Base** — su id `rol:` pasa `isRealShortTermApiMovement` y el corte de "no futuro" del Base lo respeta explícitamente porque el viaje ya ocurrió, sólo la cobranza es futura. Ver `buildScenarioForecastRun` en scenarioForecastRun.ts.
...
1. Base movements from `source.movements` (real CXP + cobranza + ROL CITI + payroll + compras + recurring providers + manual entries). For the Base run, this is filtered to real short-term API only — see "Base scenario invariant" below.
...
3. Tax movements seeded from the post-adjustment view (`buildTaxDashboardView` → `buildApprovedTaxPaymentMovements` + `buildAutomaticTaxReserveMovements`). Reserves are split into **weekly installments** between `max(asOfDate, period start)` and `dueDate` so the bulto fiscal del día 17 no tira la caja a negativo — overdue obligations still emit a single movement at `asOfDate`.
...
- **No future, excepto ROL.** The Base run drops any movement whose `effectiveMovementDate` is `> today` (cut in `buildScenarioForecastRun`, scenarioForecastRun.ts) — EXCEPT `rol:` lines. `rol:` are executed trips with a future collection date dictated by the catalog payment rule (synced from the cobranza API), so they are real money already earned and are kept in Base. The `projectionEndDate` of the Base run is `max(today, max(rol.projectedDate))` so the bucket window covers them. `cxc:`/`purchase:`/`payroll:` records whose projected payment date is in the future are still dropped — only `rol:` is exempt.
- **Real short-term API data only.** Base does NOT read the full canonical projection. `buildScenarioRun` filters `source.movements` through `isRealShortTermApiMovement` (FinancialPlanningDashboard.tsx) so Base keeps only: real JDE cobranza (`cxc:` — facturas abiertas de viajes ejecutados), ROL CITI (`rol:` — viajes ejecutados aún no facturados, regla de cobro del catálogo), JDE purchase orders/receipts (`purchase:` / `po:` — *órdenes de compra*) and real TRESS payroll (`payroll:` without `:forecast:` — *nómina*). It drops rule-projected collections (`client:`), CXP (`cxp:`), recurring providers (`recurring-*`), budget reserve (`budget-opex-gap:`), the synthetic canonical balancer (`canonical-*`) and synthetic payroll fill. The shared canonical engine is NOT modified — the cut lives only in the Base run, so Dashboard/Proyección still get the full projection. Don't move this filter into `canonicalProjection.ts`.
